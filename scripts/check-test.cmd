...
docker compose exec -T postgres psql -U abm_bot -d abm_bot -P pager=off -c "SELECT id, odoo_task_id, answer_date, selected_stage_id, selected_state, left(coalesce(progress_text, ''), 45) AS progress, left(coalesce(result_url, ''), 45) AS result_url FROM daily_answers ORDER BY updated_at DESC LIMIT 5;"
echo.
