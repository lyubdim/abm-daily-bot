...
Переменная нужна только для первичной загрузки первого PM. После его входа команда
`/invite <email или Odoo ID> <member|pm|tech_lead>` создаёт одноразовые приглашения без
перезапуска сервиса. В PostgreSQL сохраняется только SHA-256 хеш ссылки; срок действия
составляет 7 дней, а Odoo-профиль закрепляется за первым открывшим её Telegram-аккаунтом.
